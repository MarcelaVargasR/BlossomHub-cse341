    ...
    try {
        ...
        if (req.params.id === 'me') {
            if (!req.user || !req.user.id) {
                return res.status(401).json({ success: false, error: 'Not authorized, no user session found.' });
            }
        ...
        if (req.user && req.user.role !== 'admin' && user._id.toString() !== req.user.id) {
...
exports.updateUser = async (req, res, next) => {
    try {
        ...
        if (req.body.role && req.user.role !== 'admin' && req.body.role !== user.role) {
            return res.status(403).json({ success: false, error: 'Not authorized to change user role' });
        }

        // Fields allowed for update by a user themselves (if not admin)
        const allowedUpdates = ['displayName', 'profilePicture', 'phoneNumber', 'address']; // Add/remove fields as needed
        const updates = {};
